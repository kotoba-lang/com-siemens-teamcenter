(ns siemens-teamcenter.main
  "Kotodama WASM entrypoint for the Siemens_teamcenter clean-room actor (L4) — Clojure port.
  ...
  facts (`siemens_teamcenter.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "siemens_teamcenter")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"siemens_teamcenter.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "siemens_teamcenter-compat" :tier tier :entities entities} 200])
...
(defn start! [] :siemens_teamcenter-compat/ready)
