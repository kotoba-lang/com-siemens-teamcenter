(ns siemens-teamcenter.main-test
  "Contract + behavioral test for the siemens_teamcenter-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "siemens_teamcenter-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'siemens-teamcenter.main-test)]
